...
function formulate_gmcnf(; verbose = true)
    ...
    @test outflow_edges == Dict{Int8, Array{Int8}}(4=>[2], 2=>[3, 1], 3=>[1])
    inflow_edges = make_edge_dict(sink_nodes, source_nodes)
    @test inflow_edges == Dict{Int8, Array{Int8}}(3=>[2], 1=>[2, 3], 2=>[4])
    ...
    requirements_inflow_const = @expression(
        ...
        if haskey(inflow_edges, i)
            sum(
                sum(requirements_inflow[h, i, m, k] for m in 1:num_comm) * inflow[h, i, k]
